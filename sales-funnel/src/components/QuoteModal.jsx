...
export default function QuoteModal({ cotacao, onFechar, onAtualizada, onDeletada }) {
  ...
  const [fVendedor, setFVendedor] = useState(cotacao.vendedor)
  ...
  const handleSalvarEdicao = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/cotacoes/${cotacao.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...editForm, valor: parseFloat(editForm.valor) || null }),
      })
  ...
  const handleDeletar = async () => {
    if (!confirm(`Excluir cotação de ${cotacao.cliente}? Esta ação não pode ser desfeita.`)) return
    try {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100" style={{ borderTop: `4px solid ${stage.color}` }}>
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-xl font-bold text-gray-900 truncate">{cotacao.cliente}</h2>
              <p className="text-sm text-gray-500">{cotacao.vendedor}</p>
            </div>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            {cotacao.produto && <span className="text-sm text-gray-500">{cotacao.produto}</span>}
            {cotacao.data_cotacao && <span className="text-sm text-gray-400">{formatDate(cotacao.data_cotacao)}</span>}
          </div>
        ...
        <div className="flex border-b border-gray-100 px-6">
          {[
            ...
            { id: 'followup', label: '📞 Registrar Follow-up' },
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
          {erro && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
              ⚠️ {erro}
              <button onClick={() => setErro(null)} className="ml-2 text-red-400">✕</button>
            </div>
          )}
          ...
          {aba === 'historico' && (
            <div>
              {loading ? (
                <p className="text-gray-400 text-center py-8">Carregando histórico...</p>
              ) : historico.length === 0 ? (
          ...
          {aba === 'followup' && (
            <form onSubmit={handleAddFollowup} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={salvando || !fDescricao.trim()}
          ...
          {aba === 'resposta' && (
            <form onSubmit={handleAddResposta} className="space-y-4">
              <div className="p-4 bg-green-50 border border-green-100 rounded-lg">
                <p className="text-sm text-green-700 font-medium">💬 Registre o retorno que o cliente deu</p>
                <p className="text-xs text-green-600 mt-1">Anote o que o cliente respondeu, sua posição sobre a proposta, objeções ou próximos passos combinados.</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  O que o cliente disse? <span className="text-red-500">*</span>
                </label>
                <textarea
                  value={rResposta}
                  ...
                  required
                  rows={5}
                  placeholder="Ex: Cliente disse que vai analisar a proposta até sexta-feira. Pediu desconto de 10% e entrega em 15 dias..."
              ...
              <button
                type="submit"
                disabled={salvando || !rResposta.trim()}
              ...
              >
                {salvando ? 'Salvando...' : '💬 Registrar Resposta do Cliente'}
              </button>
            </form>
          )}
            ...
            <form onSubmit={handleSalvarEdicao} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={editForm.data_cotacao}
                    onChange={e => setEditForm(f => ({ ...f, data_cotacao: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setEditForm(f => ({ ...f, vendedor: e.target.value }))}
                  required
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                />
              </div>
